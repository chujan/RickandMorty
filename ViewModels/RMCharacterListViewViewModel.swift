...
class RMCharacterListViewViewModel: NSObject {
    ...
    private var apiInfo: RMGetAllCharacterResponse
        .Info? = nil
    func fetchCharacter() {
    ...
    public func fetchAdditionalCharacters(url: URL) {
        ...
        RMService.shared.excute(request, expecting: RMGetAllCharacterResponse.self) { [weak self] result in
            ...
            case.success(let responseModel):
                ...
                let originalCount = strongSelf.charcters.count
                ...
                strongSelf.charcters .append(contentsOf: moreResults)
...
extension RMCharacterListViewViewModel: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        guard shouldShowLoadMoreIndicator, !isLoadingMoreCharacters, let nextUrlString = apiInfo?.next, let url = URL(string: nextUrlString) else {
            return
        }
